          ...
          {allBands.map(band => {
            ...
            return (
              <tr key={band}>
                ...
                <td style={{ ...s.td, ...s.tdR, fontSize:13, color:"var(--text-b)" }}>{actualPct.toFixed(1)}%</td>
                <td style={{ ...s.td, ...s.tdR, fontWeight:600, fontSize:13 }}>{normPct.toFixed(1)}%</td>
                <td style={{ ...s.td, ...s.tdR, color:"var(--text-b)", fontSize:13 }}>{targetPct.toFixed(1)}%</td>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ ...s.card, padding:"12px 16px" }}>
        <div style={{ fontSize:11, fontWeight:600, letterSpacing:0.5, textTransform:"uppercase", color:"var(--text-b)", marginBottom:10 }}>Location mix</div>
        <div style={{ display:"grid", gridTemplateColumns:"80px 1fr 1fr 1fr 1fr", gap:10, marginBottom:12, alignItems:"center" }}>
          <div style={{ background:"var(--row-alt)", borderRadius:6, padding:"8px 12px", gridRow:"1", textAlign:"center" }}>
            <div style={{ fontSize:10, color:"var(--text-m)", marginBottom:2 }}>Total</div>
          ...
          {[
            ...
            <div key={label} style={{ background: "var(--row-alt)", borderRadius:6, padding:"8px 12px", border: faded ? "0.5px dashed var(--border)" : "0.5px solid var(--border)" }}>
              <div style={{ fontSize:10, color:"var(--text-m)", marginBottom:2 }}>{label}</div>
              <div style={{ display:"flex", alignItems:"baseline", gap:6 }}>
                <span style={{ fontSize:16, fontWeight:700, color: faded ? "var(--text-b)" : color }}>{pctVal.toFixed(0)}%</span>
                <span style={{ fontSize:12, color:"var(--text-b)", fontWeight:500 }}>{count}</span>
                <span style={{ fontSize:10, color:"var(--text-m)" }}>{sub}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:12 }}>
          <span style={{ color:"var(--text-b)" }}>Onshore gap:</span>
          ...
          <span style={{ fontSize:11, color:"var(--text-m)" }}>— level mix below uses target pool ({targetOnCount} on · {targetOffCount} off) as denominator</span>
        </div>
      </div>
      ...
      <div style={{ ...s.card, padding:14 }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          ...
          >
            ...
            <XAxis dataKey="band" tick={{ fontSize:10, fill:"var(--chart-tick)" }} angle={-35} textAnchor="end" interval={0} />
      ...
      <div style={{ ...s.card, padding:"16px 20px" }}>
        <div style={{ fontSize:13, fontWeight:700, letterSpacing:0.4, textTransform:"uppercase",
          color:"var(--text-b)", marginBottom:16, paddingBottom:10, borderBottom:"1px solid var(--border)" }}>
          Economics
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:20 }}>
          {[
            ...
            <div key={k.label} style={{ background:"var(--row-alt)", borderRadius:8, padding:"12px 16px",
              border: k.accent ? "1px solid #D9B3FF" : "1px solid var(--border)" }}>
              <div style={{ fontSize:12, color:"var(--text-b)", marginBottom:6, textTransform:"uppercase", letterSpacing:0.4 }}>
                {k.label}
              </div>
              <div style={{ fontSize:22, fontWeight:700, color: k.accent ? "#A100FF" : "var(--text-h)" }}>
                ${k.value.toFixed(2)}<span style={{ fontSize:13, fontWeight:400, color:"var(--text-b)" }}>/hr</span>
              </div>
              <div style={{ fontSize:12, color:"var(--text-b)", marginTop:4 }}>{k.sub}</div>
            </div>
          ))}
        </div>

        <div style={{ background:"var(--row-alt)", borderRadius:6, padding:"12px 16px",
          fontFamily:"var(--font-mono)", fontSize:12, lineHeight:2.0, marginBottom:16,
          border:"1px solid var(--border)" }}>
          <div style={{ color:"var(--text-b)" }}>Blended LCR = (onshore% × onshore LCR) + (offshore% × offshore LCR)</div>
          <div style={{ color:"var(--text-h)" }}>= ({actualOnPct.toFixed(1)}% × ${parsedOnLCR.toFixed(2)}) + ({actualOffPct.toFixed(1)}% × ${parsedOffLCR.toFixed(2)})</div>
          <div style={{ color:"var(--text-h)" }}>= ${(actualOnPct/100*parsedOnLCR).toFixed(2)} + ${(actualOffPct/100*parsedOffLCR).toFixed(2)}
          </div>
...
const sectionHdrStyle = {
  ...
  color: "var(--text-b)", padding: "6px 12px 4px",
  borderBottom: "0.5px solid var(--border)", marginBottom: 4,
};
...
function PersonDetailView({ entityType, entityName, levelBand, liveDetail, onBack, onBackToLevel }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:"var(--text-b)" }}>
        <button onClick={onBack} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--text-b)", fontSize:12, padding:0, display:"flex", alignItems:"center", gap:3 }}>
        ...
        <span style={{ marginLeft:8, fontSize:11, color:"var(--text-m)" }}>— {rows.length} resource{rows.length !== 1 ? "s" : ""}</span>
      </div>

      {rows.length === 0 ? (
        <div style={{ ...s.card, padding:24, textAlign:"center", color:"var(--text-m)", fontSize:12 }}>
              ...
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={idx} style={{ background: idx%2===1 ? "var(--row-alt)" : undefined }}>
                    ...
                    <td style={{ ...s.td, fontSize:11, fontFamily:"var(--font-mono)", color:"var(--text-b)" }}>
                    ...
                    {(r.months ?? []).map((m, mi) => (
                      <td key={mi} style={{ ...s.td, ...s.tdR, fontSize:10, padding:"4px 6px",
                        color: m > 0 ? "var(--text-h)" : "var(--text-m)" }}>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      <div>
        <button
          onClick={onBack}
          style={{ background:"none", border:"none", cursor:"pointer", color:"var(--text-b)", fontSize:12, padding:0, display:"flex", alignItems:"center", gap:4 }}
        >
          <i className="ti ti-arrow-left" style={{ fontSize:13 }} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
        <div style={{ ...s.card, padding:"10px 14px" }}>
          <div style={{ fontSize:11, color:"var(--text-b)", marginBottom:2 }}>Total resources</div>
          <div style={{ fontSize:20, fontWeight:700 }}>{groupPeople}</div>
          <div style={{ fontSize:11, color:"var(--text-m)" }}>{onshoreTotal} US · {offshoreTotal} offshore</div>
        </div>
        <div style={{ ...s.card, padding:"10px 14px" }}>
          <div style={{ fontSize:11, color:"var(--text-b)", marginBottom:2 }}>Onshore</div>
          <div style={{ fontSize:20, fontWeight:700, color:US_COL }}>{onPct}%</div>
          <div style={{ fontSize:11, color:"var(--text-m)" }}>{onshoreTotal} US resources</div>
        </div>
        <div style={{ ...s.card, padding:"10px 14px" }}>
          <div style={{ fontSize:11, color:"var(--text-b)", marginBottom:2 }}>Offshore</div>
          <div style={{ fontSize:20, fontWeight:700, color:OFF_COL }}>{offPct}%</div>
          <div style={{ fontSize:11, color:"var(--text-m)" }}>{offshoreTotal} offshore resources</div>
        </div>
        <div style={{ ...s.card, padding:"10px 14px" }}>
          <div style={{ fontSize:11, color:"var(--text-b)", marginBottom:2 }}>Staffed days</div>
...
export default function StaffingTab({ view, setView, staffing, isLive, loading, storedName, storedDate, liveDetail, monthLabels, margin, setMargin }) {
      ...
      {view === "levels" && (() => {
        ...
        return (
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ ...s.card, padding:"8px 12px", display:"flex", gap:20, fontSize:11, color:"var(--text-b)" }}>
                  ...
                  {visiblePods.map(p => {
                    ...
                    return (
                      <tr key={p.name}
                        ...
                        style={{ cursor: hasDetail ? "pointer" : "default" }}>
                        <td style={s.td}>
                          <span style={{ fontSize:10, background:"var(--row-alt)", color:"var(--text-b)", padding:"1px 6px", borderRadius:4, marginRight:6 }}>{p.group.slice(0,12)}</span>
                          {p.name}
                        </td>
                        ...
                        <td style={{ ...s.td, ...s.tdR, color:"var(--text-b)" }}>{pct(pDays, totalDays)}</td>
                        <td style={{ ...s.td, ...s.tdR, color: p.us/p.people>0.5?"#10B981":"var(--text-b)" }}>
  ...
  return (
    <div style={{ padding: "20px 24px", flex: 1, background: "var(--bg-app)", minHeight: "100vh" }}>
