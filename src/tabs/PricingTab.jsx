...
function PersonDetailView({ entityType, entityName, levelBand, locationKey, liveDetail, onBack, onBackLabel, onBackToLevel }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      {/* Breadcrumb */}
      <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#444444", flexWrap:"wrap" }}>
        <button onClick={() => onBack("top")} style={{ background:"none", border:"none", cursor:"pointer", color:"#444444", fontSize:13, padding:0 }}>
        ...
        <span style={{ marginLeft:8, fontSize:12, color:"#888888" }}>— {rows.length} resource{rows.length !== 1 ? "s" : ""}</span>
      </div>

      {rows.length === 0 ? (
        <div style={{ ...s.card, padding:24, textAlign:"center", color:"#888888", fontSize:13 }}>
        ...
        <div style={{ ...s.card, overflow:"hidden" }}>
          <div style={{ overflowX:"auto" }}>
            <table style={{ ...s.tbl, minWidth:1100 }}>
              ...
              <tbody>
                {rows.map((r, idx) => (
                  <tr key={idx} style={{ background: idx%2===1 ? "#F5F5F5" : undefined }}>
                    ...
                    <td style={{ ...s.td, fontSize:12, fontFamily:"var(--font-mono)", color:"#888888" }}>
                      {r.eid ?? <span style={{ color:"#AAAAAA" }}>TBD</span>}
                    </td>
...
function GroupLevelView({ groupName, groupLevels, billByBand, margin, DAYS, liveDetail, onBack }) {
  ...
  const LevelSection = ({ title, color, rows, locationKey }) => {
    ...
    return (
      <div style={{ ...s.card, overflow:"hidden" }}>
        <div style={{ padding:"8px 14px", borderBottom:"1px solid #E0E0E0", fontSize:12, fontWeight:600,
            ...
            {rows.map(l => {
              ...
              return (
                <tr key={l.band} style={{ cursor:"pointer" }}
                  ...
                  <td style={{ ...s.td, ...s.tdR, fontSize:13, fontWeight:500, color:"#444444" }}>{fmtHr(bill)}</td>
                  <td style={{ ...s.td, ...s.tdR, fontSize:13 }}>{Math.round(lPpl)}</td>
                  <td style={{ ...s.td, ...s.tdR, fontSize:13, color:"#444444" }}>{fmtN(Math.round(days))}</td>
                  ...
                  <td style={{ ...s.td, ...s.tdR, fontSize:12, color:"#CCCCCC" }}><i className="ti ti-chevron-right" /></td>
                </tr>
              );
            })}
            <tr style={{ background:"#F5F5F5" }}>
              <td style={{ ...s.td, fontWeight:600, fontSize:13 }}>Subtotal</td>
              <td style={{ ...s.td, ...s.tdR, fontSize:13, color:"#888888" }}>blended</td>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#444444" }}>
        <button onClick={onBack} style={{ background:"none", border:"none", cursor:"pointer", color:"#444444", fontSize:13, padding:0 }}>
          <i className="ti ti-arrow-left" style={{ fontSize:13, marginRight:4 }} />
          Pricing
        </button>
        <span>/</span>
        ...
        <span style={{ marginLeft:8, fontSize:12, color:"#888888" }}>— click a level to view individual resources</span>
      </div>

      {/* Summary strip */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
        {[
          { label:"Total cost", value:fmtM(totalCost), color:"#000000" },
          { label:"Price (" + margin.toFixed(1) + "% margin)", value:fmtM(totalCost * price), color:"#A100FF" },
          { label:"On / Off split", value:`${fmtM(onCost)} / ${fmtM(offCost)}`, color:"#444444" },
        ].map(k => (
...
export default function PricingTab({ staffing, liveDetail, margin, setMargin }) {
  ...
  const KPI = ({ label, value, sub, accent }) => (
    <div style={{ ...s.kpi, borderTop: accent ? "3px solid #A100FF" : "3px solid transparent" }}>
      ...
      <div style={{ fontSize:22, fontWeight:700, color: accent ? "#A100FF" : "#000000", marginBottom:2 }}>{value}</div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:10, alignItems:"start" }}>
        ...
        <div style={{ background:"#FFFFFF", border:"1px solid #E0E0E0", borderRadius:8,
          padding:"10px 14px", boxShadow:"0 1px 3px rgba(0,0,0,0.06)", minWidth:220 }}>
          <div style={{ fontSize:11, color:"#888888", textTransform:"uppercase", letterSpacing:0.4, marginBottom:6 }}>
            Target margin %
          </div>
          <input
            ...
            style={{ width:"100%", padding:"6px 10px", fontSize:18, fontWeight:700,
              border:"1px solid #D9B3FF", borderRadius:6, color:"#000000",
              background:"#FAFAFA", outline:"none", marginBottom:8 }}
          />
          <div style={{ fontSize:11, color:"#888888", textTransform:"uppercase", letterSpacing:0.4, marginBottom:3 }}>
            Derived price/hr
          </div>
      ...
      <div>
        <div style={{ fontSize:14, fontWeight:700, marginBottom:10, color:"#000000" }}>
          Pricing by role group
          <span style={{ fontSize:12, fontWeight:400, color:"#888888", marginLeft:10 }}>click a row to drill into level detail</span>
        </div>
        <div style={{ ...s.card, overflow:"hidden" }}>
          <table style={s.tbl}>
            <thead>
      ...
      <div>
        <div style={{ fontSize:14, fontWeight:700, marginBottom:10, color:"#000000" }}>
          Pricing by level
          <span style={{ fontSize:12, fontWeight:400, color:"#888888", marginLeft:10 }}>click a level to view individual resources</span>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
